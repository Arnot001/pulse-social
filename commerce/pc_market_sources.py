...
def market_diagnostics() -> list[str]:
    """Return a copy of diagnostics from the most recent market collection."""
...
def _query(fp: PCFingerprint) -> str:
    return " ".join(x for x in (fp.cpu, fp.gpu, f"{fp.ram_gb}GB" if fp.ram_gb else "", "gaming PC") if x)


def _same_domain(host: str, domain: str) -> bool:
    host = host.lower().split(":", 1)[0].removeprefix("www."); domain = domain.lower().removeprefix("www.")
...
def _links_from_html(html: str, base_url: str, domain: str) -> list[str]:
    candidates: list[str] = []
    patterns = (r'href\s*=\s*["\']([^"\']+)["\']', r'content\s*=\s*["\']([^"\']+)["\']', r'["\'](?:url|canonical_url|productUrl|product_url)["\']\s*:\s*["\']([^"\']+)["\']')
...
def _jsonld_products(html: str, retailer: str, domain: str, page_url: str = "") -> list[MarketListing]:
    ...
    for raw in re.findall(r'<script[^>]+type=["\']application/ld\+json["\'][^>]*>(.*?)</script>', html, re.I | re.S):
        ...
        while stack:
            ...
            if title and 250 <= price <= 15000: found.append(MarketListing(retailer=retailer, title=title, price=price, url=url))
    return found
...
def _visible_product(html: str, retailer: str, page_url: str) -> list[MarketListing]:
    ...
    if not title or not re.search(r'(?i)(gaming\s*pc|desktop|computer|ryzen|geforce|radeon|rtx|rx\s*\d)', title + " " + body[:15000]): return []
    prices = []
...
def _retailer_search_urls(domain: str, query: str) -> list[str]:
    q = quote_plus(query)
    return [f"https://www.{domain}/search?q={q}", f"https://www.{domain}/search?query={q}", f"https://www.{domain}/search/{q}", f"https://{domain}/search?q={q}"]


def _discover_urls(domain: str, query: str) -> tuple[list[str], list[str]]:
    urls: list[str] = []; notes: list[str] = []
...
def collect_market_references(fp: PCFingerprint) -> list[MarketListing]:
    LAST_DIAGNOSTICS.clear(); query = _query(fp); results: list[MarketListing] = []; seen: set[tuple[str, str, float]] = set()
    for retailer, domain in RETAILERS.items():
        ...
        accepted = 0; fetched = 0
        for url in urls[:12]:
            try: html = _fetch(url, timeout=12); fetched += 1
            except Exception: continue
            items = _jsonld_products(html, retailer, domain, url) or _visible_product(html, retailer, url)
            for item in items:
